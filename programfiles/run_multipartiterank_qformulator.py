...
def construct_galago_query(translate_top_k, keywords_with_score, QT_obejct, out_lang='en', combine_opt='sdm'): # combine or sdm
    weights_str = '#combine'
    words_str = ''
    phrase_index = 0
    for score, phrase in keywords_with_score:
        if out_lang=='ar':
            wrapped_phrase = QT_obejct.tranlate_phrase_combine(phrase, tran_top_k=translate_top_k, trans_type='tt_syn_op')
        else:
    ...
    for task in eng_dry_run: #'task-num', 'task-docs', 'task-title', 'task-stmt', 'requests', 'task-narr'
        ...
        if 'task-title' in task:
            if task['task-title'] is not None:
                ...
                if len(task_title)>3:
                    if args.out_lang=='ar':
                        cur_task_galago_query = myQT.tranlate_phrase_combine(task_title, tran_top_k=translate_top_k, trans_type='tt_syn_op')
                    else:
        ...
        if 'task-stmt' in task:
            if task['task-stmt'] is not None:
                ...
                if len(task_stmt)>3:
                    if args.out_lang=='ar':
                        cur_task_galago_query = myQT.tranlate_phrase_combine(task_stmt, tran_top_k=translate_top_k, trans_type='tt_syn_op')
                    else:
        ...
        if 'task-narr' in task:
            if task['task-narr'] is not None:
                ...
                if len(task_narr)>3:
                    if args.out_lang=='ar':
                        cur_task_galago_query = myQT.tranlate_phrase_combine(task_narr, tran_top_k=translate_top_k, trans_type='tt_syn_op')
                    else:
        ...
        cur_task_galago_query = construct_galago_query(translate_top_k,extracted_kws, myQT, args.out_lang, combine_opt=cur_formulate_op)
        ...
        for req_obj in task['requests']: #'req-docs', 'req-text', 'req-num'
            ...
            cur_req_galago_query = construct_galago_query(translate_top_k, extracted_kws_req, myQT, args.out_lang, combine_opt=cur_formulate_op)
            ...
            if len(cur_req_hl_text.strip())>3:
                if args.out_lang=='ar':
                    cur_req_hl_galago_query = myQT.tranlate_phrase_combine(cur_req_hl_text, tran_top_k=translate_top_k, trans_type='tt_syn_op')
                else:
            ...
            if 'req-text' in req_obj:
                if req_obj['req-text'] is not None:
                    ...
                    if len(cur_req_text)>3:
                        if args.out_lang=='ar':
                            cur_req_text_galago_query = myQT.tranlate_phrase_combine(cur_req_text, tran_top_k=translate_top_k, trans_type='tt_syn_op')
                        else:
