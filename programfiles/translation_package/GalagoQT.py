...
class GalagoQT:
    ...
    def construct_galago_query_no_phrasing(self, keywords_with_score):
        if len(keywords_with_score)>1:
            weights_str = '#combine'
            words_str = ''
            phrase_index = 0
            for score, phrase in keywords_with_score:
                wrapped_phrase = phrase.strip()
                if len(wrapped_phrase)>=1:
                    words_str += ' ' + wrapped_phrase
                    weights_str += f':{phrase_index}={score}'
                    phrase_index+=1
            final_query = f"{weights_str}({words_str})"
        elif len(keywords_with_score)==1:
            final_query = keywords_with_score[0][1]
        else:
            print('ERROR!! -> empty keywords list!! ')
        return final_query
    ...
    def tranlate_phrase_combine(self, ph_txt, tran_top_k=2, trans_type='tt'):
        #kw_tokens = self.spacy_english_tokenizer_and_cleaner(ph_txt)
        kw_tokens = ph_txt.split()
        wrapped_kw = ''
        if len(kw_tokens)>=1:
            kw_tokens_translations = []
            for kw_token in kw_tokens:
                ...
                wrapped_trans = self.construct_galago_query_no_phrasing(q_trans_tokens)
                kw_tokens_translations.append((0.2, wrapped_trans))
            wrapped_kw = self.construct_galago_query_no_phrasing(kw_tokens_translations)
